...
struct virt_queue *virtq_init(unsigned index) {
    ...
    virtio_reg_write32(VIRTR_QUEUE_SEL_O, index);
    // Notify the device about the queue size
    virtio_reg_write32(VIRTR_QUEUE_NUM_O, VIRTQ_ENTRY_NUM);
    // Notify the device about the used alignment
    virtio_reg_write32(VIRTR_QUEUE_ALIGN_O, 0);
    // Write the physical number of the first page of the queue
    virtio_reg_write32(VIRTR_QUEUE_PFN_O, virtq_paddr);
    return vq;
}
...
void virtq_kick(struct virt_queue *vq, int desc_index) {
    ...
    vq->avail.idx++;
    __sync_synchronize();
    virtio_reg_write32(VIRTR_QUEUE_NOTIFY_O, vq->queue_index);
    vq->last_used_index++;
}
